      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col" style={{ width: '30%', minWidth: '260px' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1.5">
              <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: degraded ? '#F59E0B' : '#10B981',
                display: 'inline-block', animation: 'pulse-soft 2s ease-in-out infinite' }} aria-hidden="true" />
              <span style={{ fontSize: '10px', color: degraded ? '#F59E0B' : '#10B981', fontWeight: 600 }}>
                {degraded ? 'Dégradé' : 'Temps réel'}
              </span>
            </div>
          </div>
          <div className="flex flex-col gap-2 overflow-y-auto flex-1" style={{ maxHeight: 'calc(100vh - 220px)' }}>
            {sortedTickets.map((t) => (
              <TicketCard key={t.id} ticket={t} onChangePriority={handleChangePriority} onTransfer={setTransferTarget} />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <h2 className="mb-3" style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0' }}>
            Guichets
            <span className="ml-2 px-1.5 py-0.5 rounded-full text-xs font-bold"
              style={{ backgroundColor: 'rgba(16,185,129,0.15)', color: '#10B981', fontFamily: 'JetBrains Mono, monospace' }}>
              {onlineCount} actifs
            </span>
          </h2>
